...
//! 4. Run the `client_credentials` exchange (no browser needed).
...
/// Writes `app.json` with the given OAuth credentials to `<config_dir>/bitbucket-cli/app.json`.
...
pub fn write_app_config(config_dir: &Path, client_id: &str, client_secret: &str) -> Result<(), CliError> {
    let dir = config_dir.join("bitbucket-cli");
...
pub fn run_init(client_id: Option<String>, client_secret: Option<String>) -> Result<(), CliError> {
    ...
    println!("\napp.json written to {}", cfg_dir.join("bitbucket-cli").join("app.json").display());
    ...
    println!("\nSetup complete. Run `bitbucket auth whoami` to verify your identity.");
    Ok(())
}
