...
// `auth.rs` is a thin wrapper over `atlassian_auth` — the OAuth flows, PKCE
// helpers, and callback parsing it delegates to are covered by that crate's
// own test suite. These tests only guard the two things that are actually
