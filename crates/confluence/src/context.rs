...
//!   Confluence API: load config → load credentials → refresh if expired →
//!   build client. Centralised here so each command handler calls one
//!   function instead of repeating the load/refresh/build chain.
...
use crate::client::{ClientError, ConfluenceClient};
...
/// Loads and auto-refreshes OAuth credentials, then builds an authenticated Confluence client.
/// Returns a clear error if the user is not logged in or the session has expired.
pub fn authenticated_client() -> Result<ConfluenceClient, CliError> {
...
/// Shared by every command handler that calls a `ConfluenceClient` method.
