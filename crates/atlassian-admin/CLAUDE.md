...
- **Why this is its own crate, not folded into `jira` or `bitbucket`**: this is a genuinely different service (different host, different auth model, different product) from either — see root `CLAUDE.md`'s "one crate per external service" convention. It was *not* a case for the shared `atlassian-auth` library either (see root `CLAUDE.md`'s "Shared library: crates/atlassian-auth") — that library is specifically about the OAuth patterns `jira`/`confluence` duplicate — this crate has no OAuth at all.
