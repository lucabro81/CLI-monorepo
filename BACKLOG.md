...
### LIB-1 — RESOLVED: shared library for Atlassian-product OAuth/config code
...
**Resolved:** 2026-08-03, when `crates/confluence` was added — confirmed live that Confluence Cloud authenticates against the *exact same* platform as Jira (`auth.atlassian.com`/`api.atlassian.com`, identical 3LO+PKCE and `client_credentials` grants, identical `cloud_id` resolution via accessible-resources), making it a byte-for-byte third copy of `jira`'s `auth.rs`, not just a similarly-shaped one. Extracted `crates/atlassian-auth` (workspace-local lib) covering exactly that: `OAuthConfig`, `Credentials`, PKCE helpers, callback parsing, `login`/`login_client_credentials`/`refresh`/`renew`/`load_credentials`/`save_credentials`/`get_granted_scopes`, parameterized by `scopes` and `cli_dir` so each crate supplies its own. `jira`'s `auth.rs` was refactored to a thin wrapper over it (all 87 existing tests still pass, no call-site changes needed in `commands/`/`context.rs`); `confluence`'s `auth.rs` follows the identical pattern. See root `CLAUDE.md`'s "Shared library: crates/atlassian-auth".  
**Deliberately still out of scope: `bitbucket`.** Re-reading its `auth.rs` during this work confirmed it's a genuinely different auth model (its own native OAuth consumer at `bitbucket.org/site/oauth2/access_token`, HTTP Basic auth instead of a JSON body, `client_credentials`-only with no PKEC/refresh_token, no `cloud_id` concept, a `scopes: Vec<String>` field instead of a `cloud_id` in `Credentials`) — forcing it into `atlassian-auth`'s API would mean bending that API around a shape it wasn't designed for, the exact speculative-abstraction root `CLAUDE.md` warns against. `atlassian-admin` was already confirmed out of scope for the same category of reason (static API key, no OAuth grant) when it was built. If a *fourth* crate shows up needing bitbucket's exact flow, revisit then — until such a case exists, this was correctly bounded to jira+confluence, not "all Atlassian-family auth."
...
### GCHAT-6 — RESOLVED: `spaces create` verified live; deliberately no automated e2e coverage
**Found:** 2026-07-21, during `spaces create` implementation
**Context:** `spaces create --user <user>` wraps `spaces.setup` to create/find a `DIRECT_MESSAGE` or `GROUP_CHAT` space by user identifier, needing a new scope (`chat.spaces.create`) not previously granted. Implementation, unit tests (`build_setup_space_body`, `normalize_user_name`), `cargo test`, and `cargo clippy` were all green from the start; live verification was blocked for a while (see below) but is now done.
**Live-verification saga, 2026-07-21**: `chat.spaces.create` was added and saved in Google Auth Platform's "Data Access" config for `mercury-500017` (confirmed correctly persisted after a full page reload — "I tuoi ambiti sensibili" listed it with the right description). Several `auth login --user` re-consents after that still showed only the original 7 scopes, both in the granted token (checked via `tokeninfo`) and in the consent screen's permission list itself. Root cause turned out to be neither the scope string, the code, nor Google-side propagation: the local OAuth callback server (`localhost:8080`) had a stale process still bound to the port from an earlier, never-completed login attempt, so every subsequent `auth login --user` was silently failing with `Address already in use (os error 48)` before ever writing a new `credentials.json` — the operator was repeatedly re-checking the *same* stale token, not a fresh one. Once that stale process was gone, `auth login --user` succeeded on the first real attempt and the new token immediately included `chat.spaces.create`.
**Verified live, 2026-07-21**: `spaces create --user <own email>` correctly rejected with `400 INVALID_ARGUMENT` ("Don't specify the calling user as a membership") when targeting the authenticated identity's own email — confirms the API call itself is well-formed and scope-authorized, just not a valid way to test a self-DM. `spaces create --user mauro.seno@comperio.it` (a colleague with a pre-existing DM, `spaces/ud85UsAAAAE`) returned that exact existing space (`{"name": "spaces/ud85UsAAAAE", "spaceType": "DIRECT_MESSAGE", ...}`) instead of creating a duplicate — idempotency confirmed live, with zero new visible state created (deliberately chosen so as not to notify/surprise anyone, per the operator having already contacted everyone in the workspace). Creating a genuinely *new* DM (first-time contact) and `GROUP_CHAT` (2+ users) were not separately exercised live — no safe test target for either without contacting someone new, which wasn't done.
**Add when:** a new-contact DM or a `GROUP_CHAT` needs live verification — requires an explicit, asked-for test target, same caution as `messages send`/`messages delete` (GCHAT-2). No automated e2e test is planned for this command specifically, for the same reason.
**Current behaviour:** untested against the live API. In particular, unverified: the exact required scope (`chat.spaces.create` vs the broader `chat.spaces` also mentioned in Google's docs), whether `GROUP_CHAT` creation behaves as documented (2+ memberships, no `displayName`), and the DM idempotency behavior (calling it twice with the same user returning the same space).
**Add when:** re-consent is done and a safe test target (a colleague who knows test spaces might be created, or a self-DM) is designated — then run `spaces create --user <target>` live, confirm the returned `Space`'s `spaceType`/`name`, and mark this resolved in `crates/google-chat/CLAUDE.md`'s Implemented-commands table. No automated e2e test is planned for this command specifically, for the same reason `messages send`/`delete` have none (GCHAT-2) — it creates real state visible to a real person.